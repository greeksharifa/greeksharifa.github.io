...
$n=4$ 인 세션을 예로 들어보자. 위와 같은 Subgraph가 있다고 할 때, $\mathbf{A_s}$ 는 위와 같이 표현된다. 이 때 $\mathbf{A}_{s, i:} \in \mathbb{R}^{1 \times 2n}$ 는 $\mathbf{A}_{s} \in \mathbb{R}^{n \times 2n}$ 의 일부이며 형광색으로 표시한 부분이 위 예시에서 $\mathbf{A}_{s, 2:}$ 가 될 것이다.  
