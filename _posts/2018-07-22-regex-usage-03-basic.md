...
2. 3번 예시는 **나태 정량자**를 보여준다. 나태 정량자는 별다른 것은 없고, 단지 정량자 바로 뒤에 `?`를 붙여주기만 하면 된다. 그러면 탐욕적 정량자였던 `*`는 최대로 일치시키는 대신 문자열은 가장 적게 먹어치우면서 일치되도록 하는 방법을 찾는다. 그래서 딱 'part 1'까지만 일치되고, 나머지 문자열은 버려진다.
    - `+?`, `{3, 5}?`, `??` 등도 가능하다.
