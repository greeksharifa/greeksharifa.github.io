...
$$ \text{video clip} \ v=\lbracev_i\rbrace^{N_v}_{i=1}, \quad \text{subtitle} \ s=\lbraces_i\rbrace^{N_s}_{i=1}$$
...
15%의 frame을 순서를 섞으면 모델은 원래 시간순서(timestamp) $ \mathbf{t} =  \lbrace t_i \rbrace^R_{i=1}, \ t_i \in  \lbrace1, ..., N_v  \rbrace$  를 알아내야 한다. 
