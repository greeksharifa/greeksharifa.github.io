...
본 Chapter는 아래 Reference에 명시되어 있는 2번째 논문의 일부를 번역한 내용을 포함한 것임을 밝힌다.  
...
이전 Chapter에서 보았던 내용이 신뢰를 얻기 위해서는 사실 우리사 얼마나 공분산 함수를 잘 선택하느냐에 달려있다. $\theta = [l, \sigma_f, \sigma_n]$ 라는 **Parameter** 집합이 적절히 설정되어야만 결과가 합리적일 것이다.  

$\theta$ 의 **Maximum a Posteriori Estimate**는 $p(\theta | \mathbf{x}, \mathbf{y})$ 가 최댓값을 가질 때 찾을 수 있다. **베이즈 정리**에 따라 우리가 $\theta$ 에 대해 거의 아는 것이 없다고 가정할 때 우리는 다음과 같은 식을 최대화해야 한다.  
...
---
## 3. Gaussian Process in Machine Learning  
사실 지난 Chapter에서 **GP**에 대한 대략적인 감은 잡았을 것이다. 이번 Chapter에서는 [이 논문](https://www.cs.ubc.ca/~hutter/EARG.shtml/earg/papers05/rasmussen_gps_in_ml.pdf)을 바탕으로 좀 더 정교하게 설명을 진행해보고자 한다.  

본 논문은 **GP**가 회귀를 위한 Bayesian 프레임워크를 형성하기 위해 어떻게 사용되는지, Random(Stochastic) Process가 무엇이고 이것이 어떻게 지도학습에 사용되는지를 설명하는 것이 주 목적이다. 또한 이전 Chapter에서 궁금증을 남겼던 공분산 함수의 **Hyperparameter** 설정에 관한 부분, 그리고 주변 우도와 Automatic Occam's Razor에 관한 이야기도 포함한다.  

(Occam's Razor: 오캄의 면도날 원칙, 단순함이 최고다.)

### 3.1. Posterior Gaussian Process  
**GP**의 유한한 버전인 다변량 정규분포는 **베이지안 추론**의 **Prior**로 사용된다. 이 **Prior**는 학습 데이터에 의존하지 않으며 함수들에 대한 어떤 특성을 구체화한다. **Posterior Gaussian Process**의 목적은 학습데이터가 주어졌을 때, 이 **Prior**를 업데이트하는 방법을 도출해내는 것이다. 나아가 새로운 데이터가 주어졌을 때 적절히 예측 값을 반환하는 것이 목표가 될 것이다.  

이전 Chapter에서 본 것처럼, 기존의 학습데이터와 새로운 테스트 데이터를 분리하여 다음과 같은 결합 분포를 상정해보자.  
...
자 이제 학습 데이터의 **Noise**를 고려해야 한다. 이전 Chapter에서도 살펴 보았듯이 이에 대해서 정규 분포를 설정하는 것이 일반적이다. Noise를 고려한 후 다시 정리하면 아래와 같다. 

<center><img src="/public/img/Machine_Learning/2020-07-12-Gaussian Process/09.JPG" width="70%"></center>  

이제 **Posterior Process**에서 샘플링을 진행할 수 있다. 이렇게 평균 함수와 공분산 함수를 정의함으로써 학습 데이터가 주어졌을 때 **Prior**를 **Posterior**로 업데이트할 수 있게 되었다. 그러나 이전 Chapter에서 맞닥드렸던 문제가 다시 한 번 찾아왔다. 어떻게 평균 함수와 공분산 함수를 적절히 설정하는가? 그리고 Noise Level은 어떻게 추정하는가?  
...
### 3.3. Conclustions and Future Directions  
...
4) [Tensorflow Github](https://github.com/tensorflow/probability/blob/master/tensorflow_probability/examples/jupyter_notebooks/Gaussian_Process_Regression_In_TFP.ipynb?fbclid=IwAR1GBOFx9znZVisebDIEv9jPLmBX7KfTCESOli7eUOQLEQdl83do9mdu5ys)  
