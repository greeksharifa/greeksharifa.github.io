...
```python
matchObj = re.fullmatch("You[;']re studying re module[.,]", 'You;re studying re module,')
print(matchObj)
```
결과
```
...
['11aa']
```
첫 번째 출력 결과는 매칭되는 문자열은 두 군데로 '12'와 '34'이다. 하지만 **re.search**는 제일 처음 하나만 찾아내기 때문에 하나믄 반환하였다.
...
(주: 단어 경계는 여기서 쓸모가 없다. 이유는 여러분이 알아서 생각하면 된다.)
