...
Downstream task로의 응용은 ERNIE 3.0을 사전학습된 공유 네트워크 및 task-specific 네트워크를 결합한 것으로 최ㄱ화하여 특정 task의 data를 사용하여 실행하는 과정을 따른다.
...
3. 공유 네트워크보다 더 작은 크기의 task-specific 네트워크는 task-spefici 표현모듈만을 미세조정했을 때보다 대규모 사전학습 모델을 위한 실용적, 실제적 응용에 더 적합하다.
...
보통의 지식 텍스트 예측 작업은 아래 그림에서 볼 수 있다.

<center><img src="/public/img/2021-07-28-ERNIE 3.0/02.png" width="100%" alt="ERNIE"></center>


지식 그래프의 triplet 쌍과 백과사전에서 얻은 대응되는 문장이 주어지면, 모델은 head/Tail Entity를 탐지하고 대응 문장에서 그것들의 관계를 판별해야 한다. 이 과정의 본질은 관계 추출 task에서 원격 지도 알고리즘과 비슷하다. 원격감독 알고리즘은 두 entity가 관계에 참여하면, 해당 entity를 포함하는 어떤 문장이든 관계를 표현할 것이라고 가정한다.  
