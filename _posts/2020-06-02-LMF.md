...
<center><img src="/public/img/Machine_Learning/2020-06-02-LMF/02.JPG" width="50%"></center>  
...
Implicit 라이브러리는 [이곳](https://github.com/benfred/implicit)에서 확인할 수 있다.  

### 2.1.Data Preparation  
**LMF** 알고리즘을 사용하기 위해서는 Sparse Matrix를 생성해주어야 한다. `scipy` 라이브러리를 통해 Sparse Matrix를 만들어보자.  
...
### 2.2. Hyper Parameter Optimization with HyperOpt and Training  
hyperopt 라이브러리를 통해 Hyperparameter 튜닝을 간단히 해보자. 본 라이브러리에 대한 정보를 조금 더 얻고 싶다면 [이전 글](2020-06-01-LightFM.md)을 보거나, 라이브러리 깃헙에 들어가서 확인하길 바란다.  

본 라이브러리를 학습하는 데에 있어 중요한 점은, 잠재 벡터의 크기를 늘릴 수록 학습 시간이 오래 걸린다는 점과(당연하다.) Iteration 수를 무작정 늘린다고 능사가 아니라는 것이다. 과적합을 떠나서 반복 횟수가 지나칠 경우 빠르게 수렴하는 본 알고리즘의 특성 상 잠재 벡터의 원소들이 모두 발산하는 아주 나쁜 결과로 이어질 수 있다. 따라서 **적절한 반복 횟수**를 찾는 것이 학습에 있어 매우 중요하다.  
...
### 2.3. 결과 확인  
[이전 글](2020-06-01-LightFM.md)과 같이 유사한 책에 대한 정보를 확인해보자.  
```python
