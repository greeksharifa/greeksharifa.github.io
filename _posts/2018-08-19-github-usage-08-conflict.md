...
1. 동시에 같은 파일의 같은 부분을 수정하고, merge 혹은 push를 할 때 일어난다. 이 뜻은 같은 파일을 수정했다 하더라도 명확히 다른 부분이 수정되었다면 git이 알아서 병합 과정을 처리해준다는 뜻이다. 
...
그리고 3rd-branch로 이동하여 비슷하게 반복한다. 
...
이제 편집기로 가서 `first.py`를 살펴보라. 에러 표시줄이 잔뜩 보일 것이다. 메모장 코딩을 한다면 에러 같은 걸 표시해 주지는 않겠지만.
...
[다음 글](https://greeksharifa.github.io/references/2018/07/13/it-will-update-soon/)에서는 무엇을 알아볼 지 정하지 않았다.
