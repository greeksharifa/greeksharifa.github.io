...
해당 튜토리얼에서는 
1. **D를 training하는 과정: D와 G를 모두 사용은 하지만 D의 parameter만 업데이트한다.**
    ...
    6. 이를 `d_steps`만큼 반복한다.
2. **G를 training하는 과정: D와 G를 모두 사용은 하지만 G의 parameter만 업데이트한다.**
    1. G로 Fake data를 생성한다.
    2. D에게서 판별 결과를 받아온다.
    3. G가 error를 계산하게 한다.
    4. G의 parameter를 업데이트한다. 
    5. 이 과정을 `g_steps`만큼 반복한다.
3. 이 전체 과정을 `num_epochs`만큼 반복한다. 해당 튜토리얼에서는 5000번 반복한다.
